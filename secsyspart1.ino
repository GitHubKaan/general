...
void loop() {
  ...
  if (first_start == true) {
    for (int i = 0; i < 5; i++) {
      digitalWrite(led_green, HIGH); digitalWrite(sound_alarm, HIGH);
      delay(50);
      digitalWrite(led_yellow, HIGH); digitalWrite(led_green, LOW); digitalWrite(sound_alarm, LOW);
      delay(50);
      digitalWrite(led_red, HIGH); digitalWrite(led_yellow, LOW);
      delay(50);
      digitalWrite(led_red, LOW);
    }
    digitalWrite(sound_alarm, HIGH); digitalWrite(led_green, HIGH); digitalWrite(led_yellow, HIGH);  digitalWrite(led_red, HIGH);
    delay(300);
    digitalWrite(sound_alarm, LOW); digitalWrite(led_green, LOW); digitalWrite(led_yellow, LOW);  digitalWrite(led_red, LOW);
    first_start = false;
  ...
  if (digitalRead(button_unarm) == LOW && system_unarmed == false) {
    if (final_alarm == false) {
      digitalWrite(device_signal, HIGH);
      delay(500);
      digitalWrite(device_signal, LOW);
    }
  ...
  if (time_type1 == 2) {
   ...
   //[]==========[10 sec]==========[]
  if (time_type2 == 10) {
    time_type2 = 0; //reset time_type2 (time_type2: 10 = 10 sec)
    //--- /\ --- time settings --- /\ ---
