...
void loop() {
  ...
  if (digitalRead(button_unarm) == LOW && system_unarmed == false) {
    ...
    for (int i = 0; i < 2; i++) {
      digitalWrite(sound_alarm, HIGH); digitalWrite(led_green, HIGH); digitalWrite(led_yellow, HIGH);  digitalWrite(led_red, HIGH); delay(50); digitalWrite(sound_alarm, LOW); digitalWrite(led_green, LOW); digitalWrite(led_yellow, LOW);  digitalWrite(led_red, LOW); delay(50);
    }
